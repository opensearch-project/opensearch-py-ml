...
class TestSeriesArithmetics(TestData):
    def test_invalid_add_num(self):
        ...
        with pytest.raises(TypeError):
            assert (
                self.ed_ecommerce()["currency"] + self.ed_ecommerce()["total_quantity"]
            )

        with pytest.raises(TypeError):
            assert (
                self.ed_ecommerce()["total_quantity"] + self.ed_ecommerce()["currency"]
            )

    def test_ser_add_ser(self):
        edadd = (
            self.ed_ecommerce()["customer_first_name"]
    ...
    def test_non_aggregatable_add_aggregatable(self):
        with pytest.raises(ValueError):
            assert (
                self.ed_ecommerce()["customer_gender"]
                + self.ed_ecommerce()["customer_first_name"]
            )
