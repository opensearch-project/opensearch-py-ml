...
model_uploader = MLCommonModelUploader(OPENSEARCH_TEST_CLIENT)


def test_check_mandatory_field():
    model_meta = {}
    with pytest.raises(ValueError, match="Model metadata can't be empty"):
        model_uploader.check_mandatory_field(model_meta)
    model_meta = {
    ...
    assert True == model_uploader.check_mandatory_field(model_meta)
