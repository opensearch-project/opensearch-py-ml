...
class TestModelRegister(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher_iam_role_helper = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.IAMRoleHelper')
        self.MockIAMRoleHelper = self.patcher_iam_role_helper.start()

        self.patcher_ai_connector_helper = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.AIConnectorHelper')
        self.MockAIConnectorHelper = self.patcher_ai_connector_helper.start()

        # Patch model classes
        self.patcher_bedrock_model = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.BedrockModel')
        self.MockBedrockModel = self.patcher_bedrock_model.start()

        self.patcher_openai_model = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.OpenAIModel')
        self.MockOpenAIModel = self.patcher_openai_model.start()

        self.patcher_cohere_model = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.CohereModel')
        self.MockCohereModel = self.patcher_cohere_model.start()

        self.patcher_huggingface_model = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.HuggingFaceModel')
        self.MockHuggingFaceModel = self.patcher_huggingface_model.start()

        self.patcher_custom_pytorch_model = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.CustomPyTorchModel')
    ...
    def test_initialize_clients_success(self, mock_boto_client):
        ...
        model_register = ModelRegister(self.config, self.mock_opensearch_client, self.opensearch_domain_name)
    ...
    def test_prompt_model_registration_register_new_model(self, mock_input):
        model_register = ModelRegister(self.config, self.mock_opensearch_client, self.opensearch_domain_name)
        with patch.object(model_register, 'register_model_interactive') as mock_register_model_interactive:
    ...
    def test_prompt_model_registration_use_existing_model(self, mock_input):
        ...
        with patch.object(model_register, 'save_config') as mock_save_config:
            model_register.prompt_model_registration()
            self.assertEqual(model_register.config['embedding_model_id'], 'model-id-123')
    ...
    @patch('builtins.input', side_effect=['1'])
    @patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.ModelRegister.initialize_clients', return_value=True)
    def test_register_model_interactive_bedrock(self, mock_initialize_clients, mock_input):
        self.MockIAMRoleHelper.return_value.get_iam_user_name_from_arn.return_value = 'test-user'
    ...
    @patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.ModelRegister.initialize_clients', return_value=True)
    def test_register_model_interactive_openai_managed(self, mock_initialize_clients, mock_input):
        model_register = ModelRegister(self.config, self.mock_opensearch_client, self.opensearch_domain_name)
    ...
    @patch('builtins.input', side_effect=['2'])
    @patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.ModelRegister.initialize_clients', return_value=True)
    def test_register_model_interactive_openai_opensource(self, mock_initialize_clients, mock_input):
        model_register = ModelRegister(self.config, self.mock_opensearch_client, self.opensearch_domain_name)
    ...
    @patch('builtins.input', side_effect=['invalid'])
    @patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.model_register.ModelRegister.initialize_clients', return_value=True)
    def test_register_model_interactive_invalid_choice(self, mock_initialize_clients, mock_input):
        model_register = ModelRegister(self.config, self.mock_opensearch_client, self.opensearch_domain_name)

        with patch('builtins.print') as mock_print:
            model_register.register_model_interactive()
            mock_print.assert_called_with('\x1b[31mInvalid choice. Exiting model registration.\x1b[0m')

    @patch('builtins.input', side_effect=['1'])
    def test_prompt_opensource_model_registration_register_now(self, mock_input):
        model_register = ModelRegister(self.config, self.mock_opensearch_client, self.opensearch_domain_name)
        with patch.object(model_register, 'register_model_opensource_interactive') as mock_register:
    ...
    def test_prompt_opensource_model_registration_register_later(self, mock_input):
        ...
        with patch('builtins.print') as mock_print:
            model_register.prompt_opensource_model_registration()
            mock_print.assert_called_with('Skipping model registration. You can register models later using the appropriate commands.')
    ...
    def test_prompt_opensource_model_registration_invalid_choice(self, mock_input):
        ...
        with patch('builtins.print') as mock_print:
            model_register.prompt_opensource_model_registration()
            mock_print.assert_called_with('\x1b[31mInvalid choice. Skipping model registration.\x1b[0m')
    ...
    def test_register_model_opensource_interactive_huggingface(self, mock_input):
        ...
        self.MockHuggingFaceModel.return_value.register_huggingface_model.assert_called_once_with(
            model_register.opensearch_client, model_register.config, model_register.save_config
        )

    @patch('builtins.input', side_effect=['1'])
    def test_register_model_opensource_interactive_no_opensearch_client(self, mock_input):
        model_register = ModelRegister(self.config, None, self.opensearch_domain_name)
        with patch('builtins.print') as mock_print:
            model_register.register_model_opensource_interactive()
            mock_print.assert_called_with('\x1b[31mOpenSearch client is not initialized. Please run setup again.\x1b[0m')
