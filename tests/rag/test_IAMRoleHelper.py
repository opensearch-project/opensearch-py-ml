...
class TestIAMRoleHelper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        logger = logging.getLogger("opensearch_py_ml.ml_commons.IAMRoleHelper")
        logger.setLevel(logging.CRITICAL)  # Suppress logs below CRITICAL during tests

    @patch("opensearch_py_ml.ml_commons.IAMRoleHelper.boto3.client")
    ...
    def test_delete_role_success(self, mock_boto_client):
        ...
        mock_iam_client.detach_role_policy.assert_called_with(
            RoleName="test-role", PolicyArn="arn:aws:iam::aws:policy/ExamplePolicy"
        )
        mock_iam_client.delete_role_policy.assert_called_with(
            RoleName="test-role", PolicyName="InlinePolicy"
        )
    ...
    def test_create_iam_role_success(self, mock_boto_client):
        ...
        role_arn = self.iam_helper.create_iam_role(
            "test-role", trust_policy, inline_policy
        )
    ...
    def test_create_iam_role_error(self, mock_boto_client):
        ...
        role_arn = self.iam_helper.create_iam_role(
            "existing-role", trust_policy, inline_policy
        )
    ...
    @patch("opensearch_py_ml.ml_commons.IAMRoleHelper.requests.put")
