...
class TestRAGCLI(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher_load_config = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.rag.load_config', return_value=self.mock_config)
        self.mock_load_config = self.patcher_load_config.start()
        self.addCleanup(self.patcher_load_config.stop)

        self.patcher_save_config = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.rag.save_config')
        ...
        self.patcher_setup = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.rag.Setup')
        self.mock_setup_class = self.patcher_setup.start()
        self.addCleanup(self.patcher_setup.stop)

        self.patcher_ingest = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.rag.Ingest')
        self.mock_ingest_class = self.patcher_ingest.start()
        self.addCleanup(self.patcher_ingest.stop)

        self.patcher_query = patch('opensearch_py_ml.ml_commons.rag_pipeline.rag.rag.Query')
    ...
    def test_setup_command(self):
        ...
        with patch.object(sys, 'argv', test_args):
            ...
            self.mock_save_config.assert_called_once_with(self.mock_setup_class.return_value.config)
    ...
    def test_ingest_command_with_paths(self):
        ...
        with patch.object(sys, 'argv', test_args):
            ...
            self.mock_ingest_class.return_value.ingest_command.assert_called_once_with(['/path/to/data1', '/path/to/data2'])
    ...
    def test_ingest_command_without_paths(self):
        ...
        with patch.object(sys, 'argv', test_args):
            with patch('rich.prompt.Prompt.ask', side_effect=['/path/to/data', '']):
                ...
                self.mock_ingest_class.assert_called_once_with(self.mock_config)
                self.mock_ingest_class.return_value.ingest_command.assert_called_once_with(['/path/to/data'])

    def test_query_command_with_queries(self):
        test_args = ['rag.py', 'query', '--queries', 'What is OpenSearch?', 'How does Bedrock work?']
        with patch.object(sys, 'argv', test_args):
            main()
            ...
            self.mock_query_class.return_value.query_command.assert_called_once_with(
                ['What is OpenSearch?', 'How does Bedrock work?'],
                num_results=5
            )
    ...
    def test_query_command_without_queries(self):
        ...
        with patch.object(sys, 'argv', test_args):
            with patch('rich.prompt.Prompt.ask', side_effect=['What is OpenSearch?', '']):
                main()
                # Ensure Query.query_command() is called with prompted queries
                self.mock_query_class.assert_called_once_with(self.mock_config)
                self.mock_query_class.return_value.query_command.assert_called_once_with(['What is OpenSearch?'], num_results=5)

    def test_query_command_with_num_results(self):
        test_args = ['rag.py', 'query', '--queries', 'What is OpenSearch?', '--num_results', '3']
        with patch.object(sys, 'argv', test_args):
            main()
    ...
    def test_no_command(self):
        ...
        with patch.object(sys, 'argv', test_args):
            ...
            self.assertTrue("usage: rag.py" in stderr_output or "usage: rag.py" in stdout_output)
    ...
    def test_invalid_command(self):
        ...
        with patch.object(sys, 'argv', test_args):
            ...
            print("STDERR:", stderr_output)
            print("STDOUT:", stdout_output)
            self.assertTrue("invalid choice: 'invalid'" in stderr_output or "invalid choice: 'invalid'" in stdout_output)
