...
class TestServerless(unittest.TestCase):
    ...
    @patch(
        "opensearch_py_ml.ml_commons.rag_pipeline.rag.serverless.Serverless.create_access_policy"
    )
    @patch(
        "opensearch_py_ml.ml_commons.rag_pipeline.rag.serverless.Serverless.create_security_policy"
    )
    def test_create_security_policies_success(
        self, mock_create_security_policy, mock_create_access_policy
    ):
    ...
    def test_create_security_policy_success(self):
        ...
        with patch("builtins.print") as mock_print:
            self.serverless.create_security_policy(
                policy_type, name, description, policy_body
            )
            self.aoss_client.create_security_policy.assert_called_with(
                description=description, name=name, policy=policy_body, type=policy_type
            )
            mock_print.assert_called_with(
                f"{Fore.GREEN}Encryption Policy '{name}' created successfully.{Style.RESET_ALL}"
            )
    ...
    def test_create_security_policy_conflict(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_called_with(
                f"{Fore.YELLOW}Network Policy '{name}' already exists.{Style.RESET_ALL}"
            )
    ...
    def test_create_security_policy_exception(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_called_with(
                f"{Fore.RED}Error creating {policy_type} policy '{name}': Invalid policy type specified.{Style.RESET_ALL}"
            )
    ...
    def test_create_collection_success(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_called_with(
                f"{Fore.GREEN}Collection '{self.collection_name}' creation initiated.{Style.RESET_ALL}"
            )
    ...
    def test_create_collection_conflict(self):
        ...
        self.serverless.get_collection_id = MagicMock(
            return_value="existing-collection-id"
        )
        with patch("builtins.print") as mock_print:
            collection_id = self.serverless.create_collection(self.collection_name)
            self.assertEqual(collection_id, "existing-collection-id")
            mock_print.assert_called_with(
                f"{Fore.YELLOW}Collection '{self.collection_name}' already exists.{Style.RESET_ALL}"
            )
    ...
    def test_create_collection_exception_retry(self):
        ...
        with patch("builtins.print"):
            collection_id = self.serverless.create_collection(
                self.collection_name, max_retries=3
            )
            self.assertEqual(collection_id, "collection-id-123")
    ...
    def test_get_collection_id_not_found(self):
        self.aoss_client.list_collections.return_value = {
            "collectionSummaries": [{"name": "other-collection", "id": "other-id"}]
        }
        collection_id = self.serverless.get_collection_id(self.collection_name)
    ...
    def test_wait_for_collection_active_success(self):
        ...
        with patch("builtins.print"):
            result = self.serverless.wait_for_collection_active(
                collection_id, max_wait_minutes=1
            )
    ...
    def test_wait_for_collection_active_timeout(self):
        ...
        with patch("builtins.print"):
            result = self.serverless.wait_for_collection_active(
                collection_id, max_wait_minutes=0.01
            )
    ...
    def test_get_collection_endpoint_success(self):
        ...
        self.aoss_client.batch_get_collection.return_value = {
            "collectionDetails": [
                {"collectionEndpoint": "https://example-endpoint.com"}
            ]
        }
        with patch("builtins.print"):
