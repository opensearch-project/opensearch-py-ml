...
from opensearch_py_ml.ml_commons.rag_pipeline.rag.ml_models.SageMakerModel import (
    SageMakerModel,
)
...
class TestSageMakerModel(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            self.sagemaker_model.opensearch_domain_name, self.opensearch_domain_name
        )
        self.assertEqual(
            self.sagemaker_model.opensearch_username, self.opensearch_username
        )
        self.assertEqual(
            self.sagemaker_model.opensearch_password, self.opensearch_password
        )
        self.assertEqual(
            self.sagemaker_model.iam_role_helper, self.mock_iam_role_helper
        )
    ...
    def test_register_sagemaker_model(self, mock_input):
        ...
        self.sagemaker_model.register_sagemaker_model(
        ...
        inline_policy = next(
            arg for arg in call_args if isinstance(arg, dict) and "Statement" in arg
        )
        self.assertEqual(
            inline_policy["Statement"][0]["Action"], ["sagemaker:InvokeEndpoint"]
        )
        self.assertEqual(
        ...
        connector_input = next(
            arg for arg in call_args if isinstance(arg, dict) and "name" in arg
        )
