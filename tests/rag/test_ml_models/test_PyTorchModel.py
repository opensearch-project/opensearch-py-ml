...
class TestCustomPyTorchModel(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.custom_pytorch_model.opensearch_domain_name, self.opensearch_domain_name)
        self.assertEqual(self.custom_pytorch_model.opensearch_username, self.opensearch_username)
        self.assertEqual(self.custom_pytorch_model.opensearch_password, self.opensearch_password)
        self.assertEqual(self.custom_pytorch_model.iam_role_helper, self.mock_iam_role_helper)
    ...
    def test_register_custom_pytorch_model_default(self, mock_file, mock_isfile, mock_input):
        ...
        self.custom_pytorch_model.register_custom_pytorch_model(mock_opensearch_client, mock_config, mock_save_config)

        self.assertEqual(mock_opensearch_client.transport.perform_request.call_count, 4)
        mock_save_config.assert_called_once_with({'embedding_dimension': 768, 'embedding_model_id': 'registered_model_id'})

    @patch('builtins.input', side_effect=['2', '/path/to/model.pt', '{"name": "custom_model", "model_format": "TORCH_SCRIPT", "model_config": {"embedding_dimension": 512, "framework_type": "CUSTOM", "model_type": "bert"}, "description": "Custom model"}'])
    @patch('os.path.isfile', return_value=True)
    @patch('builtins.open', new_callable=mock_open, read_data=b'model_content')
    def test_register_custom_pytorch_model_custom(self, mock_file, mock_isfile, mock_input):
        mock_opensearch_client = Mock()
        mock_opensearch_client.transport.perform_request.side_effect = [
            {'model_id': 'uploaded_model_id'},
            {'task_id': 'test-task-id'},
        ...
        self.assertEqual(mock_opensearch_client.transport.perform_request.call_count, 4)
        mock_save_config.assert_called_once_with({'embedding_model_id': 'registered_model_id'})

    @patch('builtins.input', side_effect=['1', '/nonexistent/path.pt'])
    @patch('os.path.isfile', return_value=False)
    def test_register_custom_pytorch_model_file_not_found(self, mock_isfile, mock_input):
        mock_opensearch_client = Mock()
        mock_config = {}
        mock_save_config = Mock()
    ...
    def test_wait_for_model_registration_success(self, mock_sleep, mock_time):
        ...
        result = self.custom_pytorch_model.wait_for_model_registration(mock_opensearch_client, 'test-task-id')
    ...
    def test_wait_for_model_registration_timeout(self, mock_sleep, mock_time):
        ...
        result = self.custom_pytorch_model.wait_for_model_registration(mock_opensearch_client, 'test-task-id', timeout=5)
        self.assertIsNone(result)
