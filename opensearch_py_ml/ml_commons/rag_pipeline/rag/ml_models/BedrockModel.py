...
class BedrockModel:
    ...
    def register_bedrock_model(self, helper, config, save_config_method):
        ...
        if not connector_id:
            print(f"{Fore.RED}Failed to create Bedrock connector. Aborting.{Style.RESET_ALL}")
            return
        ...
        if not model_id:
            print(f"{Fore.RED}Failed to create Bedrock model. Aborting.{Style.RESET_ALL}")
            return

        # Save model_id to config
        self.save_model_id(config, save_config_method, model_id)
        print(f"{Fore.GREEN}Bedrock model registered successfully. Model ID '{model_id}' saved in configuration.{Style.RESET_ALL}")
