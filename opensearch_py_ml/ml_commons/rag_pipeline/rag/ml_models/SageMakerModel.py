...
class SageMakerModel:
    ...
    def register_sagemaker_model(self, helper, config, save_config_method):
        ...
        sagemaker_region = (
            input(f"Enter your SageMaker region [{self.aws_region}]: ").strip()
            or self.aws_region
        )
        ...
        if not connector_id:
            print(
                f"{Fore.RED}Failed to create SageMaker connector. Aborting.{Style.RESET_ALL}"
            )
            return
        ...
        if not model_id:
            print(
                f"{Fore.RED}Failed to create SageMaker model. Aborting.{Style.RESET_ALL}"
            )
            return
        ...
        print(
            f"{Fore.GREEN}SageMaker model registered successfully. Model ID '{model_id}' saved in configuration.{Style.RESET_ALL}"
        )
