...
class Setup:
    ...
    def configure_aws(self):
        ...
        aws_region_input = (
            input(f"Enter your preferred AWS region [{self.aws_region}]: ").strip()
            or self.aws_region
        )
        ...
        try:
            ...
            subprocess.run(
                ["aws", "configure", "set", "region", aws_region_input], check=True
            )
    ...
    def setup_configuration(self):
        ...
        if self.service_type == "managed":
            ...
            self.aws_region = (
                input(f"\nEnter your AWS Region [{self.aws_region}]: ").strip()
                or self.aws_region
            )
            self.iam_principal = (
                input(f"Enter your IAM Principal ARN [{self.iam_principal}]: ").strip()
                or self.iam_principal
            )
            ...
            self.opensearch_password = self.get_password_with_asterisks(
            ...
            self.opensearch_endpoint = (
                input(
                    f"\nPress Enter to use the default endpoint (or type your custom endpoint) [{default_endpoint}]: "
                ).strip()
                or default_endpoint
            )
            auth_required = (
            ...
            if auth_required == "yes":
                self.opensearch_username = input(
        ...
        self.config = {
            ...
            "opensearch_endpoint": (
                self.opensearch_endpoint if self.opensearch_endpoint else ""
            ),
            "opensearch_username": (
                self.opensearch_username if self.opensearch_username else ""
            ),
            "opensearch_password": (
                self.opensearch_password if self.opensearch_password else ""
            ),
        }
        ...
        if (
            ...
            model_choice = input(
                f"\nEnter the number of your chosen model (1-{len(available_models)}): "
            ).strip()
            try:
            ...
            try:
                maxTokenCount = int(
                    input("Enter max token count [1000]: ").strip() or "1000"
                )
            except ValueError:
                maxTokenCount = 1000
            try:
            ...
            try:
                ...
                topP = 0.9
            stopSequences_input = input(
                "Enter stop sequences (comma-separated) or press Enter for none: "
            ).strip()
        ...
        pipeline_name = input(
            f"\nEnter the name of the ingest pipeline to use [{default_pipeline_name}]: "
        ).strip()
    ...
    def initialize_clients(self) -> bool:
        ...
        try:
            ...
            self.bedrock_client = boto3.client(
                self.SERVICE_BEDROCK, region_name=self.aws_region, config=boto_config
            )
            time.sleep(2)
            print(
                f"{Fore.GREEN}AWS Bedrock client initialized successfully.{Style.RESET_ALL}\n"
            )
            return True
        except Exception as e:
            print(
                f"{Fore.RED}Failed to initialize AWS Bedrock client: {e}{Style.RESET_ALL}"
            )
            return False
    ...
    def get_opensearch_domain_info(region: str, domain_name: str) -> tuple:
        ...
        try:
            ...
            print(
                f"{Fore.RED}Error retrieving OpenSearch domain info: {e}{Style.RESET_ALL}"
            )
            return None, None
    ...
    def get_knn_index_details(self) -> tuple:
        ...
        dimension_input = input(
            "Press Enter to use the default embedding size (768), or type a custom size: "
        ).strip()
        if dimension_input == "":
        ...
        space_choice = input(
            "Enter your choice (1-3), or press Enter for default (L2): "
        ).strip()
        ...
        shards_input = input(
            "\nEnter number of shards (Press Enter for default value 1): "
        ).strip()
        if shards_input == "":
            number_of_shards = 1
        else:
        ...
        replicas_input = input(
            "\nEnter number of replicas (Press Enter for default value 2): "
        ).strip()
        if replicas_input == "":
        ...
        passage_text_field = input(
            "\nEnter the field name for text content (Press Enter for default 'passage_text'): "
        ).strip()
        ...
        passage_chunk_field = input(
            "\nEnter the field name for passage chunks (Press Enter for default 'passage_chunk'): "
        ).strip()
        ...
        embedding_field = input(
            "\nEnter the field name for embeddings (Press Enter for default 'passage_embedding'): "
        ).strip()
        if embedding_field == "":
        ...
        if self.initialize_opensearch_client():
            ...
            if index_choice == "1":
                self.index_name = input(
                    "\nEnter a name for your new KNN index in OpenSearch: "
                ).strip()
                ...
                existing_index_name = input(
                    "\nEnter the name of your existing KNN index: "
                ).strip()
                ...
                try:
                    if not self.opensearch_client.indices.exists(index=self.index_name):
                        ...
                        index_info = self.opensearch_client.indices.get(
                        ...
                        embedding_field_mappings.get("method", {})
                        ...
                        if "method" in embedding_field_mappings:
                            ...
                            ef_construction = method.get("parameters", {}).get(
                                "ef_construction", 512
                            )
            ...
            self.model_register = ModelRegister(
                self.config, self.opensearch_client, self.opensearch_domain_name
            )

            if (
                self.service_type == "managed"
                and self.config["default_search_method"] == "semantic_with_llm"
            ):
                ...
                print(
                    f"{Fore.GREEN}Setup complete. No LLM model registration required.{Style.RESET_ALL}"
                )
